...
public class FileData
{
	...
	{
		return (Convert.ToInt64(high) << 0x20) | low;
	}
...
{
	private FastDirectoryEnumerator()
	{
	}
	/// <summary>
	...
	/// </exception>
    public static IEnumerable<SharpCompr.FileData> EnumerateFiles(string path, SearchOption options)
	{
		return FastDirectoryEnumerator.EnumerateFiles(path, "*", options);
	}
	...
	/// </exception>
	public static IEnumerable<SharpCompr.FileData> EnumerateFiles(string path, string searchPattern, SearchOption options)
	{
		return FastDirectoryEnumerator.EnumerateFiles(path, searchPattern, options, 0);
	}
	...
	/// </exception>
    public static IEnumerable<SharpCompr.FileData> EnumerateFiles(string path__1, string searchPattern, SearchOption searchOption__2, int x)
	{
		if (path__1 == null) {
		...
		if ((searchOption__2 != SearchOption.TopDirectoryOnly) && (searchOption__2 != SearchOption.AllDirectories)) {
			throw new ArgumentOutOfRangeException("searchOption");
		}

		string fullPath = Path.GetFullPath(path__1);

		return new FileEnumerable(fullPath, searchPattern, searchOption__2);
	}
	...
	/// <paramref name="filter"/> is a null reference (Nothing in VB)
	/// </exception>
    public static SharpCompr.FileData[] GetFiles(string path, string searchPattern, SearchOption searchOption)
	{
        IEnumerable<SharpCompr.FileData> e = FastDirectoryEnumerator.EnumerateFiles(path, searchPattern, searchOption);
        List<SharpCompr.FileData> list = new List<SharpCompr.FileData>(e);

        SharpCompr.FileData[] retval = new SharpCompr.FileData[list.Count];
		list.CopyTo(retval);
	...
	/// </summary>
    private class FileEnumerable : IEnumerable<SharpCompr.FileData>
	{
        ...
        #region "IEnumerable<SharpCompr.FileData> Members"
		...
		/// </returns>
		//Public Function Get1Enumerator() As IEnumerator(Of AMMSync.FileData)
		...
		/// </returns>
        public System.Collections.Generic.IEnumerator<SharpCompr.FileData> GetEnumerator()
		{
			return new FileEnumerator(m_path, m_filter, m_searchOption);
		}
    ...
    private class FileEnumerator : IEnumerator<SharpCompr.FileData>
	{


		[DllImport("kernel32.dll", CharSet = CharSet.Auto, SetLastError = true)]
		private static extern SafeFindHandle FindFirstFile(string fileName, 		[In(), Out()]
...
WIN32_FIND_DATA lpFindFileData);
		...
		/// operation should include all subdirectories or only the current directory.
		/// </param>
		public FileEnumerator(string path, string filter, SearchOption searchOption__1)
		{
			m_path = path;
			m_filter = filter;
			m_searchOption = searchOption__1;
		...
		// ''' </returns>
		//'Public ReadOnly Property Current1() As FileData
		//'    Get
		...
		public bool MoveNext()
		{
			...
			if (m_currentContext.SubdirectoriesToProcess == null) {
				if (m_hndFindFile == null) {
					// FileIOPermission(FileIOPermissionAccess.PathDiscovery, m_path).Demand()
					FileIOPermission ioperm = new FileIOPermission(FileIOPermissionAccess.PathDiscovery, m_path);
					ioperm.Demand();
		...
		public SharpCompr.FileData Current {
            get { return new SharpCompr.FileData(m_path, m_win_find_data); }
		}
